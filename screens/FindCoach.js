...
dataSource: [],
  ...
  getCoach = () => {
    this.requestRef = db.collection('coach').onSnapshot((snapshot) => {
      var coachList = snapshot.docs.map((document) => document.data());
      this.setState({
        coach: coachList,
      });
    });
...
<TouchableOpacity    style={{marginTop:2}} onPress={() => {
                  this.props.navigation.navigate('CoachDetailScreen', {
                    details: item,
                  });
                }}>
