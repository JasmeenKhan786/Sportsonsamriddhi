...
export default class RequestProfile extends React.Component {
 ...
 db.collection('notification').doc(this.state.notiDocId).update({
  ...
  addNotificationAccept = (userId, playerEmail, playerName, username) => {
    var msg =
      userId +
      ' has accepted your opponent request. ';
    var date = moment().format('DD/MM/YYYY');
    var sentFrom = userId;
    var sentTo = playerEmail;
     var randomRequestId = this.createUniqueId();
        ...
        .then((snapshot) => {
          snapshot.forEach((doc) => {
            ...
            db.collection('opponents')
              .doc(doc.id)
              .update({
                "rivals": firebase.firestore.FieldValue.arrayUnion(this.state.playerEmail),
              });
          });
        });
    } catch (e) {
        ...
        <ScrollView
          ...
          <View
            ...
            <View style={{ flex: 1, marginLeft: 20, margin: 15 }}>
              ...
              <View
                  style={{ flexDirection: 'row', flexWrap: 'wrap', flex: 1, justifyContent:'space-around'}}>
                  <LinearGradient
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        this.addNotificationAccept(
                          this.state.userId,this.state.playerEmail, this.state.playerName, this.state.username)
                      }}>
                        <Icon name="checkmark-circle-outline" type="ionicon" size={25} color="white" />
                    </TouchableOpacity>
                  </LinearGradient>
 ...
 <LinearGradient
                   ...
                   marginBottom: 15,
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                          this.addNotificationDecline (this.state.userId, this.state.playerEmail, this.state.playerName, this.state.username)
